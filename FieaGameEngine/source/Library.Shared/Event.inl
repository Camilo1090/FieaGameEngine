...
namespace FieaGameEngine
{
	...
	template <typename T>
	inline Event<T>::Event(const T& message) : EventPublisher(sSubscribers), mMessage(message)
	{
	}

	template <typename T>
	inline Event<T>::Event(T&& message) : EventPublisher(sSubscribers), mMessage(std::move(message))
	{
	}
